<template>
  <div class="project_task">
    <div class="project_task_top">
      ...
      <i class="iconfont icon-star project_star"
         v-else
         @click="projectStart(starFlag)"></i>
        ...
        <div class="mainContent"
          ...
          <div class="partitionsMain noPartitions"
            ...
            <div class="thumbnailList"
                 v-if="noPartitions.autoExpand && noPartitions.partitionId==0"
                 :style="{'width':112+140+252*stageList.length +'px'}">
              <div class="iconBox_">
                <el-tooltip class="item"
                            effect="dark"
                            :open-delay="300"
                ...
                <transition name="fade">
                  ...
                  <div class="partitionsAndStages"
                    ...
                    <div ref='partitions'
                      ...
                      <!-- <div v-if='true' class="cur stageTittle">{{element.partitionTitle}}</div> -->
                      <div type="text"
                           class="stageTittle"
                           contenteditable="true"
                           @keydown='checkEnter'
                      ...
                      <transition-group class="taskListsTwo"
                        ...
                        <li v-for="(item,index) in element.taskList"
                          ...
                          <span class="stageLists cur"
                                :class="{'leftTaskFixed':leftFixed,'hoverBg':mouseLeftIndex==item.taskId}">
                            <span class="iconBox_"
                                  v-if="item.taskId!=='-1' && item.taskId.indexOf('-')==-1">
                              <el-tooltip class="item moveHandel"
                                          effect="dark"
                                          content="移动"
                                          :open-delay="300"
                            ...
                            <!-- <span class="stageName">{{item.taskTitle}}</span> -->
                            <span v-html="item.taskTitle"
                                  class="stageName"
                                  contenteditable="true"
                                  @keydown='checkEnter'
                            ...
                            <!-- <textarea class="stageName" name="" v-model="item.taskTitle">
                          </textarea> -->
                          </span>
                          <div class="stageListsBox"
                               :class="{'stageListsBoxFixed':leftFixed}">
...
export default {
  ...
  data() {
    return {
      PartitionListPage: 0, //当前默认页码
      PartitionListNums: 10,
      isLoading: true, //是否加载
      loadMoreContet: '下拉加载更多',
        ...
        if (val) {
          for (let i of val) {
            if (!i.taskList.length) {
              i.taskList.push(this.newTask);
            } else {
              var index = val.findIndex(res => {
                return res.taskList[0].taskId == -1 || res.taskList[0].taskId.indexOf('-') !== -1;
              })
            }
          }
          // val.splice(index,1);
  ...
  methods: {
    // 下拉加载更多
    scrollBottom() {
      if (((window.screen.height + this.$refs.viewBox.scrollTop) > (this.$refs.mainContent.offsetHeight)) && this.isLoading) {
        this.PartitionListPage += 1;
        // this.loadMoreContet
        this.isLoading = false;
        if (this.PartitionListPage < Math.ceil(this.allProjectNum / 10)) {
          this.getProjectAll('', this.projectId, false);
        } else {
          this.loadMoreContet = '没有更多了';
          return
        }
      }
    },
    ...
    serchPartitionAndTask(value) {
      this.getProjectAll(value, this.projectId, true);
    },
    haha(lists) {
      lists.stageHoverIsShow = true;
    ...
    closePop() {
      ...
      this.getProjectAll('', this.projectId, true);
    },
    ...
    projectChange(selectedProject) {
      this.getProjectAll('', selectedProject, true);
    ...
    partitionBlur(el, index, name) {
      ...
      if (this.isNewP) {
        if (title) {
          console.log(title, );
          this.partitionsList[index].partitionTitle = title;
          ...
          this.$HTTP('post', '/partition_iSort_add', obj).then(res => {
            ...
            this.isNewP = false;
            // this.partitionsList[index].partitionTitle = title;
            // this.partitionsList[index].partitionId = arr.partitionId;
            // this.partitionsList.splice(index, 1, res.result);
          
            console.log(res.result, this.partitionsList)
    ...
    delPartition(element, index) {
      ...
      if (!element.taskList.length) {
        ...
        for (let list of element.taskList) {
          ...
          if (list.taskId !== '-1' && list.taskId.indexOf('-') == '-1') {
    ...
    stageNameBlur(name, item, el, index, indexs) {
      ...
      if (this.isNewS) {
        ...
        if (this.nowStageName !== taskTitle && this.nowStageName) {
    ...
    datadragEnd2(elment, evt) {
      ...
      // if (evt.to.childNodes.length = 2) {
      //   // if()
      // }
      // console.log('移动了', evt.to);
    ...
    dragEnd(elment, evt) {
      if (evt.to.childNodes.length == 2) {
        let toEl1 = evt.to.childNodes[0].dataset.taskid;
        let toEl2 = evt.to.childNodes[1].dataset.taskid;
        if (toEl1 == -1 || toEl1.indexOf('-') !== -1) {
          let index = this.partitionsList.findIndex(res => {
            return res.partitionId == evt.to.dataset.partitionid;
          })
          this.partitionsList[index].taskList.splice(0, 1)
        } else {
          let index = this.partitionsList.findIndex(res => {
            return res.partitionId == evt.to.dataset.partitionid;
          })
          this.partitionsList[index].taskList.splice(1, 1)
        }
      }
    },
    ...
    // 获取项目所有列表
    getProjectAll(keys, projectId, isAdd) {
      let data = { project: projectId, 'myUserId': this.userPkid, 'keys': keys, 'pageIndex': this.PartitionListPage, 'pageSize': this.PartitionListNums };
      this.$HTTP('post', '/project_get_info2', data, $('#app')[0]).then(res => {
        ...
        if (arr.length) {
          if (isAdd) {
            ...
            arr.forEach(res => {
              this.partitionsList.push(res);
            })
          }
          this.isLoading = true;
          this.searchIsShow = false;
        } else {
          this.loadMoreContet = '没有更多数据了';
          this.isLoading = false;
    ...
    partitionsListNum() {
      ...
      this.$HTTP('post', '/project_get_info2_count', data).then(res => {
        this.allProjectNum = res.result;
        if (this.allProjectNum < 20) {
          this.loadMoreContet = '';
        }
      }
      )
  ...
  mounted() {
    ...
    $('.tableBox')[0].addEventListener('scroll', this.scrollBottom);
  }
};
</script>
